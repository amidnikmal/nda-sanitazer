...
Не коммитьте заполненный словарь. Совпадения в словаре по умолчанию чувствительны
к регистру, но устойчивы к разному пробелу между словами термина: обычный,
неразрывный (U+00A0), узкий неразрывный (U+202F), табуляция, перенос строки и
повторы — всё это матчится (частая причина «термин из вставки не заменился»).
Ведущие и хвостовые пробелы в самих значениях словаря обрезаются при загрузке.
Разные варианты написания по-прежнему добавляйте отдельными строками.

Регистр можно отключить опцией `case_insensitive=True` (см. «Опции» ниже) — тогда
`SecretProjectX` поймается и в нижнем регистре. По умолчанию опция выключена,
чтобы сохранить документированное точное совпадение по регистру.
...
- `case_insensitive` (по умолчанию `False`) - совпадение словаря без учёта
  регистра.
- `pii_score_threshold` (по умолчанию `0.0`) - минимальный балл уверенности
  Presidio; поднимите (например `0.4`), чтобы отсечь шумные срабатывания.
- `mask_unmapped_pii` (по умолчанию `False`) - маскировать также типы Presidio
  вне белого списка (например `US_SSN`, `US_PASSPORT`) под категорией `PII`,
  вместо того чтобы пропускать их без замены.

Значения по умолчанию сохраняют прежнее поведение; PII-опции влияют только на
слой Presidio. Длинный ввод перед локальным LLM-слоем разбивается на
перекрывающиеся куски, а обрезанный ответ модели (по `max_tokens`) разбирается
частично - завершённые элементы массива не теряются.
...
- Спан позднего слоя (PII/LLM), задевающий уже вставленный плейсхолдер,
  обрезается по его границам, поэтому соседние слова могут быть замаскированы «с
  запасом». Направление ошибки здесь — перемаскирование, а не утечка.
